...
namespace QEngine
{
	public class QLoadContent
	{
		...
		public void Texture(string texture, string path)
		{
			cm.LoadTexture(texture, path);
		}
		...
		public void Circle(string name, int radius, QColor? color = null)
		{
			if(color == null)
				color = QColor.White;
			var t = CreateCircle(engine.GraphicsDevice, radius, color.Value);
			cm.LoadCustomTexture(name, t);
		}
