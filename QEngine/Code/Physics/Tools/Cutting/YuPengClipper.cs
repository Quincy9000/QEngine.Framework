...
using QEngine.Physics.Tools.Cutting.Simple;
using QEngine.Physics.Tools.PolygonManipulation;
using QEngine.Physics.Utilities;

namespace QEngine.Physics.Tools.Cutting
{
    ...
    public static class YuPengClipper
    {
        ...
        {
            System.Diagnostics.Debug.Assert(subject.IsSimple() && clip.IsSimple(), "Non simple input!", "Input polygons must be simple (cannot intersect themselves).");
        ...
        private static PolyClipError BuildPolygonsFromChain(List<Edge> simplicies, out List<Vertices> result)
        {
            ...
            while (simplicies.Count > 0)
            {
                ...
                while (!closed && simplicies.Count > 0)
                {
                    ...
                    if (!closed)
                    {
                        if (++index == simplicies.Count)
                        {
                            ...
                            {
                                result = new List<Vertices>();
                                System.Diagnostics.Debug.WriteLine("Undefined error while building result polygon(s).");
                ...
                if (output.Count < 3)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("Degenerated output polygon produced (vertices < 3).");
                }
                result.Add(output);
            }
            return errVal;
